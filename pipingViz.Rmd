...
<img src="img/leafletpngbecauseleafletneverworksinslides.png" style="display:block; margin: 0 auto; width:25%">
...
```{r, eval=FALSE}
pred1 = predict(lm(mpg~hp, data=mtcars))
```
...
3. Make a scatterplot (<span class='func'>add_markers</span> ) of weight (wt) and miles per gallon (mpg) 
    - color = amFactor 
4. add (<span class='func'>add_lines</span> ) with your **prediction** object
</div>
